...
def tf_ssim(img1, img2, cs_map=False, l=False, mean_metric=True, size=3):
    window = tf.reshape(np.full((size, size), 1.0, dtype=np.float32), [size,size,1,1])
...
def tf_ms_ssim(img1, img2, level=5):
    ...
    weight = tf.fill([level], 1.0 / level)
    window = _tf_fspecial_gauss(3, 1.0)
    ml = []
    mcs = []
    for i in range(level):
        ...
        size = ceil(img1.shape[1].value / 2)
